...
jQuery(document).ready(function ($) {
  ...
  $('#get-article-button').on('click', function () {
    ...
    var customSourceUrl = $('#custom-source-url').val();
    var postID = $('#post_ID').val(); // ID-ul postării curente

    var button = $(this); // Dezactivăm butonul și adăugăm un loader
    ...
    $.ajax({
      ...
      error: function error(xhr, status, _error) {
        console.error('Eroare AJAX:', _error);
  ...
  $('#generate-image-button').on('click', function () {
    ...
    var button = $(this);
    var feedbackText = $('#feedback-text').val(); // Feedback-ul din câmpul text
  ...
  var sursaFotoElement = $('#sursa-foto');
  ...
  if (sursaFotoElement.length && sursaFotoElement.text().includes('Imagine generata AI')) {
    var infoIcon = $('<span>').text('i').css({
      ...
      marginLeft: '10px',
      cursor: 'pointer'
    }).attr('title', 'Aceasta imagine a fost generata de AI pe baza textului din rezumat si nu reprezinta realitatea surprinsa prin intermediul unei camere foto.').on('mouseenter', function () {
      var tooltip = $('<div>').text('Aceasta imagine a fost generata de AI pe baza textului din rezumat si nu reprezinta realitatea surprinsa prin intermediul unei camere foto.').css({
        position: 'absolute',
        backgroundColor: '#333',
        color: 'white',
        padding: '10px',
...
//# sourceMappingURL=auto-ai-news-poster.dev.js.map
